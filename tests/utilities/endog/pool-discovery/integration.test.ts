...
import { PoolDiscoveryService } from '../../../../src/utilities/endog/pool-discovery/core/PoolDiscoveryService';
...
import { PoolInfoExtractor } from '../../../../src/utilities/endog/pool-discovery/core/PoolInfoExtractor';
import { PoolRegistry } from '../../../../src/utilities/endog/pool-discovery/storage/PoolRegistry';
import { SubscriptionRegistry } from '../../../../src/utilities/endog/pool-discovery/storage/SubscriptionRegistry';
import { InitializeEvent } from '../../../../src/utilities/endog/pool-discovery/types/PoolManagerTypes';
...
describe('Pool Discovery Integration Tests', () => {
  ...
  describe('End-to-End Pool Discovery Flow', () => {
    ...
    it('should handle multiple subscriptions for same currency pair', async () => {
      ...
      expect(result!.matchingSubscriptions.map((s: EndogenousVariableSubscription) => s.id)).toContain('sub-1');
      expect(result!.matchingSubscriptions.map((s: EndogenousVariableSubscription) => s.id)).toContain('sub-2');
    });
  ...
  describe('Subscription Management', () => {
    ...
    it('should get pools for subscription', async () => {
      ...
      expect(pools.map((p: any) => p.poolId)).toContain('0x1111111111111111111111111111111111111111111111111111111111111111');
      expect(pools.map((p: any) => p.poolId)).toContain('0x2222222222222222222222222222222222222222222222222222222222222222');
    });
  });
